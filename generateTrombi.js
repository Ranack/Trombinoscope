  ...
  if (courses && courses.length > 0) {
    for (i = 0; i < courses.length; i++) {
      ...
      if(students && students.length > 0){
        ...
        var fileId = '1_YCxlhUtOaIx5HmPUIlg_SjolbugXazl'; //penser à changer l'id du fichier image stocké sur le drive pour avoir le logo de l'établissement
            ...
            for( a=0; a < students.length; a++){
               ...
               var studentName = student.profile.name.fullName.substring(0);
                if (x <4){
                   ...
                   table.getCell(y, x).insertImage(0, resp);
                   x++;
